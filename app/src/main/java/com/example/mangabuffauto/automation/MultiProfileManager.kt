...
class MultiProfileManager(context: Context) {
    ...
    private val profileStore: ProfileStore? =
        if (WebViewFeature.isFeatureSupported(WebViewFeature.MULTI_PROFILE)) ProfileStore.getInstance() else null
    fun isSupported(): Boolean = profileStore != null
    fun profiles(): List<String> {
        val store = profileStore ?: return listOf(DEFAULT_PROFILE)
        return store.getAllProfileNames().filter { it != "Default" }.ifEmpty { listOf(DEFAULT_PROFILE) }
    }
    fun activeProfile(): String = prefs.getString(KEY_ACTIVE_PROFILE, DEFAULT_PROFILE) ?: DEFAULT_PROFILE
    fun setActiveProfile(name: String) {
        require(name.isNotBlank())
        prefs.edit().putString(KEY_ACTIVE_PROFILE, name).apply()
        ensureProfile(name)
    }
    fun ensureProfile(name: String) { if (isSupported()) profileStore?.getOrCreateProfile(name) }
    fun createProfile(name: String): Boolean {
        val clean = name.trim()
        if (clean.isEmpty() || clean == "Default" || !isSupported()) return false
