...
/**
 * Безопасный планировщик для ОДНОГО активного WebView-профиля.
 *
 * Он не переключает аккаунты автоматически и не создаёт параллельные браузеры.
 * Это намеренно: профильная изоляция используется для ручного выбора аккаунта,
 * а автоматизация в конкретный момент работает только с активным профилем.
 *
 * Интервалы основаны на поведении MangaBuffAutoDaily 1.5.0:
 * chat — 930 секунд, periodic reading — 1 час.
 * Для шахты и рекламы используется не более одного запуска в сутки.
 */
class AutomationScheduler(
    ...
    private val finishListener: (AutomationTask, TaskResult) -> Unit = { task, result ->
        ...
        // После завершения не запускаем следующий task прямо из callback.
        // Следующий выбор делает основной scheduler tick, что исключает
        // рекурсивные цепочки и гонки с очисткой TaskManager.
    }
    ...
    fun onProfileChanged() {
        loadTimestamps()
        AndroidBotLog.log("AUTO: scheduler profile changed -> ${runtime.activeProfile()}")
    }
    ...
    private fun tickInternal() {
        ...
        if (engine.state.value == AutomationState.BLOCKED ||
            engine.state.value == AutomationState.CAPTCHA ||
            engine.state.value == AutomationState.ERROR
        ) {
            return
        }

        // Только одна задача одновременно. Никаких параллельных WebView task.
        if (engine.hasTaskInFlight()) return

        val now = System.currentTimeMillis()

        // Суточные задачи — по одной за сутки.
        if (now - lastMineAt >= DAY_MS) {
            lastMineAt = now
            saveTimestamp(KEY_MINE_AT, lastMineAt)
            launch(MineTask)
            return
        }

        if (now - lastAdsAt >= DAY_MS) {
            lastAdsAt = now
            saveTimestamp(KEY_ADS_AT, lastAdsAt)
            launch(WatchAdsTask)
            return
        }

        // Chat: 930 секунд, как в MangaBuffAutoDaily.
        if (now - lastChatAt >= CHAT_INTERVAL_MS) {
            lastChatAt = now
            saveTimestamp(KEY_CHAT_AT, lastChatAt)
            launch(ChatDiamondTask)
            return
        }

        // Reading: один запуск в час. ReadTask сама управляет переходом глав.
        if (now - lastReadAt >= READ_INTERVAL_MS) {
            lastReadAt = now
            saveTimestamp(KEY_READ_AT, lastReadAt)
            launch(ReadTask)
        }
    }
    ...
    private fun loadTimestamps() {
        ...
        lastChatAt = prefs.getLong(prefix + KEY_CHAT_AT, 0L)
        lastReadAt = prefs.getLong(prefix + KEY_READ_AT, 0L)
        lastMineAt = prefs.getLong(prefix + KEY_MINE_AT, 0L)
        lastAdsAt = prefs.getLong(prefix + KEY_ADS_AT, 0L)
    }

    private fun saveTimestamp(key: String, value: Long) {
        prefs.edit().putLong(profilePrefix() + key, value).apply()
    }

    private fun profilePrefix(): String {
        return "profile_${runtime.activeProfile()}_"
    }
