 ...
 * В каждый момент времени допускается только один активный запуск.
 * Очереди здесь намеренно нет: следующая задача стартует только явным вызовом.
 */
class TaskManager {
    ...
    @Synchronized
    fun current(): TaskExecution? = activeExecution
    ...
    @Synchronized
    fun hasActive(): Boolean = activeExecution != null
}
